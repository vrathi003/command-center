...
async def find_unbalanced_posted_transaction_ids(
    ...
    """Return posted transaction IDs that lack two balanced postings."""
    cursor = await conn.execute(
        ...
        GROUP BY tx.id
        HAVING COUNT(posting.id) < 2 OR COALESCE(SUM(posting.amount_paise), 0) != 0
        ORDER BY tx.id
        """
    )
...
async def assert_ledger_healthy(conn: aiosqlite.Connection) -> None:
    ...
    if transaction_ids:
        raise LedgerIntegrityError(f"Unbalanced posted ledger transactions: {transaction_ids}")
