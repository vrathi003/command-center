...
CREATE INDEX IF NOT EXISTS idx_recon_statements_account_period
...
CREATE INDEX IF NOT EXISTS idx_recon_statement_lines_statement_status
    ON recon_statement_lines(statement_id, status);
CREATE UNIQUE INDEX IF NOT EXISTS idx_recon_statement_lines_external_key
    ON recon_statement_lines(statement_id, external_key)
    WHERE external_key IS NOT NULL;

CREATE TABLE IF NOT EXISTS recon_matches (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    line_id INTEGER NOT NULL UNIQUE REFERENCES recon_statement_lines(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_recon_matches_ledger_transaction
